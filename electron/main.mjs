...
function registerIpc() {
  ...
  ipcMain.handle('ground:getSession', () => ({
    ...sessionWriter.snapshot(),
    connection: serialService.currentStatus(),
  }));
  ipcMain.on('ground:recordLatency', (_event, metric) => {
    if (!metric || !Number.isFinite(metric.receivedAtMs)
        || !Number.isFinite(metric.storeLatencyMs)
        || !Number.isFinite(metric.paintLatencyMs)
        || metric.storeLatencyMs < 0 || metric.paintLatencyMs < 0
        || metric.storeLatencyMs > 60000 || metric.paintLatencyMs > 60000) return;
    sessionWriter.append('renderer_latency', {
      receivedAtMs: metric.receivedAtMs,
      storeLatencyMs: metric.storeLatencyMs,
      paintLatencyMs: metric.paintLatencyMs,
    });
  });
  ipcMain.on('ground:recordAppDecodeMismatch', (_event, mismatch) => {
    if (!mismatch || mismatch.error !== 'APP_DECODE_MISMATCH'
        || !Number.isInteger(mismatch.seq) || mismatch.seq < 0 || mismatch.seq > 0xFFFFFFFF
        || !Number.isInteger(mismatch.header) || mismatch.header < 0 || mismatch.header > 255) return;
    sessionWriter.append('app_decode_mismatch', {
      seq: mismatch.seq,
  ...
  ipcMain.on('ground:rendererReload', () => {
    sessionWriter.appendConnectionEvent('renderer_reload');
  });
}
